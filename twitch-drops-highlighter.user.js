...
(function () {
    ...
    window.addEventListener("load", () => {
        ...
        // NOMBRE DE CAMPAÑA DE RECOMPENSAS -> SU ID, aparte de los de drops y por dos
        // motivos que son el mismo: CONTROL Resonant tiene una campaña de cada clase y las
        // dos se llaman «CONTROL Resonant launch» (la de drops con un espacio al final,
        // que la clave se come). En un indice comun se anularian entre si, y el enlace de
        // la de drops —que funciona hoy— dejaria de funcionar por culpa de la otra.
        //
        // Aqui NO se anula el homonimo, al reves que en los de drops: gana el primero. Las
        // tres campañas de Pokemon se llaman igual y la pagina las junta en un solo
        // acordeon, asi que sus tres ids enfocan lo mismo (verificado el 2026-09-08 con
        // `?dropID=`, ver el comentario de `campaignId` en el bucle de recompensas).
        //
        // Las de juego nulo (las sitewide) van solo por nombre; las demas, solo dentro de
        // su juego, que es lo que la caratula del acordeon permite comprobar.
        const _apiRewardCampaignIdsPorJuego = {};
        const _apiRewardCampaignIdsSinJuego = {};
        ...
        //
        // EL HASH ES EL DE HOY, y la segunda vez que esto muerde. Una persisted query
        // identifica una VERSION de la consulta: el hash viejo NO falla —responde 200, con
        // sus datos— pero devuelve los campos que aquella version pedia. Nada se rompe y
        // nada avisa; solo faltan cosas.
        //
        // Aqui faltaba `rewardGroups`, que es donde viven los ESCALONES de una campaña de
        // recompensas. Sin el, el codigo cae a `rc.rewards` + `rc.unlockRequirements`, que
        // son una sola recompensa y un solo umbral. Reportado el 2026-09-21 con CONTROL
        // Resonant: reparte Sierra Suit (60 min), Sierra Vest (120) y Sierra Helmet (240),
        // y el panel enseñaba «Sierra Helmet (4 h)» y nada mas. Coincidia con lo apuntado
        // el 2026-09-11 («rewardGroups dejo de venir»), que no era Twitch retirando el
        // campo: era este hash quedandose atras.
        //
        // Medido el 2026-09-22 en el navegador, interceptando `fetch` y comparando las dos
        // peticiones. La de la pagina lleva `69750554…` y sus cuatro campañas de
        // recompensas llegan con 1, 3, 3 y 3 grupos; la nuestra llevaba `5a4da2ab…`. Las
        // `variables` son las mismas (`fetchRewardCampaigns: true`), asi que lo unico que
        // cambiaba era la version de la consulta.
        //
        // Y una diferencia que SI hay que saber: en esta respuesta `owner` trae `{id,
        // name}` y no `login`, asi que `_gqlGetCampaignDetails` usa su respaldo `'twitch'`
        // como `channelLogin`. No es nuevo ni es un daño: ese respaldo ya estaba actuando
        // —la campaña de drops de CONTROL, cuyo owner es «Twitch Gaming», entregaba sus dos
        // tramos con nombre— porque ese parametro solo da contexto: la campaña se busca por
        // su `dropID`.
        //
        // Como se comprueba que un hash se ha quedado viejo, sin tener que sospecharlo: el
        // aviso de mas abajo, que canta en consola la campaña de recompensas que llega sin
        // `rewardGroups`. Es lo que faltaba las dos veces.
        async function _gqlGetCampaigns() {
            const body = [{
                operationName: "ViewerDropsDashboard",
                variables: { fetchRewardCampaigns: true },
                extensions: { persistedQuery: { version: 1, sha256Hash: "69750554e0a81492f2d343558f84bdf3e324767650a2dbb6e79a3c629b4548cf" } }
            }];
        ...
        // LAS RECOMPENSAS QUE SE CANJEAN CON UN CODIGO: `{ nombre, asset }` por cada una.
        //
        // Existe para que el barrido de notificaciones de Twitch NO se lleve por delante
        // el aviso que trae el canje. El resto de avisos de drop se borran porque lo que
        // dicen ya esta en el inventario —el drop es tuyo y ahi sigue— pero el de un
        // codigo es OTRA COSA: es el codigo, y borrarlo puede ser perderlo.
        //
        // Verificado el 2026-09-22 en `earnedDropRewards` de este usuario:
        //     item: { name: "Corrupted Creeper Cape", distributionType: "CODE",
        //             thumbnailURL: ".../REWARD/07bed736-….png" }
        // O sea que Twitch lo dice en el MISMO campo que ya separa `EMOTE` y `BADGE`
        // (ver AUTO_GRANTED_TYPES): un solo enum, dos sitios donde aparece.
        //
        // Se guardan las DOS señas, y con que case una basta:
        //   · el NOMBRE, porque el aviso de Twitch lo escribe («¡Recibiste el emote
        //     "Bop2bop Emote"!» es la forma conocida de esta familia de avisos);
        //   · y el ASSET de su imagen, que es un uuid y no se traduce, asi que salva el
        //     caso de que el texto del aviso no nombre la recompensa.
        // Ninguna de las dos esta comprobada CONTRA UN AVISO DE CODIGO REAL —cuando se
        // fue a mirar, esa campaña ya habia cerrado— asi que si las dos fallan el aviso
        // se borra como antes: el peor caso es el comportamiento viejo, no uno nuevo.
        let _codeRewards = [];
        ...
        async function fetchInventoryProgress() {
            try {
                ...
                let conEstadoPropio = 0;
                for (const c of campaigns) {
                    if (c?.id) campaignIdsInProgress.add(c.id);
                    // EL ESTADO, SI ESTA CONSULTA LO TRAE, y sale gratis: esta respuesta
                    // ya esta pagada. Importa porque `ViewerDropsDashboard` deja de
                    // devolver una campaña poco despues de que cierre y aun asi la
                    // campaña sigue en tu inventario (ver _preguntarEstadoDeCampaña).
                    //
                    // Solo se escribe cuando hay dato de verdad: sin `status` ni `endAt`,
                    // `_campaignStatus` contesta 'upcoming' por descarte, y eso no es una
                    // lectura sino un relleno con aspecto de dato.
                    if (c?.id && (c.status || c.endAt)) {
                        _apiCampaignStatusById[c.id] = _campaignStatus(c);
                        conEstadoPropio++;
                    }
                ...
                for (const edge of (inv?.earnedDropRewards?.edges || [])) {
                    ...
                    if (n.item?.distributionType === 'CODE' && !codigosVistos.has(rid)) {
                        codigosVistos.add(rid);
                        const asset = (String(n.item?.thumbnailURL || '')
                            .match(/REWARD\/([0-9a-fA-F-]{8,})\./) || [])[1] || '';
                        codigos.push({ nombre: _fold(nombre.toLowerCase()), asset });
                    }
                    if (cid && nombre) {
                ...
                console.log('[Twitch Drops] indice de reclamados:',
                    ...
                    '| campañas en curso que traen su propio estado', conEstadoPropio + '/' + campaigns.length);
                ...
                _codeRewards = codigos;
                // Solo cuando hay alguna: es la lista que decide que avisos NO se borran,
                // asi que poder contrastarla con lo que se ve en el centro de
                // notificaciones es lo unico que hace comprobable esa regla.
                if (codigos.length) {
                    console.log('[Twitch Drops] recompensas de CODIGO en tu historial (su aviso no se borra):',
                        codigos.length, codigos.map(c => c.nombre));
                }
        ...
        function _isDropClaimed(drop) {
            ...
            if (drop.rewardCampaign) {
                // POR NOMBRE, ANTES QUE POR CUENTA. Lo de arriba vale para las cajas, que
                // no se llaman como lo que sueltan; pero una campaña de recompensas tambien
                // puede repartir la recompensa TAL CUAL, y entonces el historial apunta
                // exactamente el nombre del tramo. Reportado el 2026-09-23 con CONTROL
                // Resonant: tres grupos —Sierra Suit, Vest y Helmet— y el Suit ya
                // concedido. Con solo la cuenta daba 1 < 3 y no se tachaba nada, mientras
                // el chip de «lo que ya te dio» pintaba «✓ Sierra Suit» al lado: el mismo
                // premio dos veces, una pendiente y otra hecha.
                //
                // Con homonimos dentro de la campaña hacen falta tantas concesiones con
                // ese nombre como tramos lo llevan: una sola «Great Ball» concedida no
                // dice cual de las tres fue, y tachar las tres seria marcar de mas.
                if (drop.campaignId && drop.name) {
                    const nombre = _fold(String(drop.name).toLowerCase());
                    const conEseNombre = (_earnedRewardsByCampaign[drop.campaignId] || [])
                        .filter(r => r && _fold(String(r.name || '').toLowerCase()) === nombre).length;
                    if (conEseNombre > 0 && conEseNombre >= (Number(drop.homonimosEnCampaña) || 1)) return true;
                }
        ...
        async function _fetchDropsViaGQL() {
            ...
            //
            // Y POR ESO ES UN MAPA Y NO UN CONJUNTO: juego -> el texto de las campañas que
            // lo metieron. Una hermana no casa por si misma, asi que con su propio texto
            // su tarjeta sale SIN NINGUN CHIP. En las de drops no se notaba porque se
            // funden en la entrada del juego, que ya lleva el texto de la que casa; pero
            // una campaña de recompensas tiene entrada propia (va por campaña, no por
            // juego) y se quedaba muda. Reportado el 2026-09-23 con CONTROL Resonant: en
            // el inventario su tarjeta no decia «twitch», y en campañas si, porque ahi la
            // fila del DOM toma el texto de la entrada de drops del juego. El texto del
            // juego solo se suma a las ETIQUETAS (`searchText` no decide nada mas: el
            // filtro ya se aplico arriba), asi que no puede meter a nadie en el panel.
            const _juegosDeInteres = new Map();
            for (const c of dropCampaigns) {
                ...
                if (!_matchesKeywords(txt)) continue;
                const previo = _juegosDeInteres.get(juego);
                _juegosDeInteres.set(juego, previo && previo.indexOf(txt) !== -1 ? previo
                    : (previo ? previo + ' ' + txt : txt));
            }
            // El texto con el que se calculan los chips de una campaña: el suyo, y si entro
            // prestada, ademas el de las campañas que metieron a su juego.
            const _textoDeEtiquetas = (searchText, gameName, hermana) =>
                hermana ? searchText + ' ' + (_juegosDeInteres.get(gameName) || '') : searchText;

            // LAS QUE LLEGAN SIN ESCALONES, para cantarlas al final. Un hash de consulta
            // que se queda viejo no da error: responde de menos, y eso se ve como «esta
            // campaña solo da una cosa», que es una frase perfectamente creible. Ha pasado
            // dos veces (los emotes en agosto, los escalones ahora) y las dos se tardo en
            // ver porque nadie lo decia. Con esto, la proxima se lee en la consola.
            const _sinEscalones = [];

            // QUE CLASE DE RECOMPENSAS ESTA REPARTIENDO TWITCH HOY, y cuales se CANJEAN.
            //
            // Esto no adorna: es el unico sitio desde el que se puede reconocer un tipo
            // de recompensa NUEVO sin que nadie lo sospeche antes. `distributionType` ya
            // decide cosas aqui —`AUTO_GRANTED_TYPES` separa lo que Twitch concede solo
            // de lo que hay que reclamar— asi que un valor que no conozcamos cambia el
            // comportamiento en silencio.
            //
            // Y el motivo inmediato (2026-09-22): hay drops que se entregan como CODIGO
            // —la pagina les pinta una pildora «CÓDIGO» y un boton «Canjear»— y se quiso
            // que su notificacion no se borrara sola. No se pudo hacer: cuando se fue a
            // medir, la campaña del ejemplo («Corrupted Creeper Cape», de Minecraft) ya
            // habia cerrado, y escribir la deteccion sin ver el dato seria adivinar entre
            // dos candidatos —un `distributionType` propio o el par
            // `redemptionURL`/`redemptionInstructions` de las campañas de recompensas—.
            // Con estas dos lineas, la proxima campaña de codigo se delata sola en la
            // consola el dia que aparezca, sin tener que volver a montar una medicion.
            const _censoTipos = {};
            const _canjeables = [];
            ...
            for (const rc of rewardCampaigns) {
                ...
                // ANTES del filtro, como los de drops: el enlace de categoria se arregla en
                // toda la pagina, no solo en las campañas que te interesan.
                const claveRc = _claveDeCampaña(campaignName);
                if (claveRc && rc.id) {
                    const gid = _gameIdOf(rc);
                    const mapa = gid ? _apiRewardCampaignIdsPorJuego : _apiRewardCampaignIdsSinJuego;
                    const k = gid ? _claveDeCampañaEnJuego(gid, claveRc) : claveRc;
                    if (!(k in mapa)) mapa[k] = rc.id;
                }
                // LA MISMA REGLA DE HERMANAS QUE LAS CAMPAÑAS DE DROPS, y por el mismo
                // motivo: el filtro es por campaña y la tarjeta es por juego. Un juego que
                // ya te interesa se lleva TODAS sus campañas abiertas, y una campaña de
                // recompensas es una campaña abierta mas de ese juego.
                //
                // Faltaba aqui porque este bucle es anterior a la regla (2026-09-20) y se
                // quedo con el filtro de antes. Lo que costaba: CONTROL Resonant reparte
                // tres cosas en su campaña de recompensas —Sierra Suit, Vest y Helmet— y la
                // fila de la pagina no podia enseñar ninguna, porque su campaña no casa con
                // ninguna keyword por si misma (el juego esta en el panel por la campaña de
                // DROPS, que casa por su owner «Twitch Gaming»). La fila acababa enseñando
                // los premios de esa otra campaña, que es lo que se reporto.
                //
                // Y no abre la puerta a todo: `_juegosDeInteres` solo tiene los juegos que
                // ya tienen tarjeta, la negativa sigue mandando, y esto no cuesta ninguna
                // consulta extra —las recompensas vienen enteras en el mismo payload, no
                // como los tramos de drops, que piden un `DropCampaignDetails` por campaña—.
                const casaSola = _matchesKeywords(searchText);
                const hermanaDeInteres = !casaSola && !!gameName &&
                    _juegosDeInteres.has(gameName) && !_hasNegativeKeyword(searchText);
                if (!casaSola && !hermanaDeInteres) continue;
                const textoEtiquetas = _textoDeEtiquetas(searchText, gameName, hermanaDeInteres);
                ...
                const tieneEscalones = !!(rc.rewardGroups && rc.rewardGroups.length);
                if (!tieneEscalones) _sinEscalones.push(campaignName || rc.id || '(sin nombre)');
                const grupos = tieneEscalones
                    ? rc.rewardGroups
                ...
                for (const g of grupos) {
                    ...
                    for (const r of (g.rewards || [])) {
                        ...
                        const _url = String(r.redemptionURL || '');
                        const _inst = String(r.redemptionInstructions || '');
                        if (_inst || (_url && !/^https:\/\/www\.twitch\.tv\/?$/.test(_url))) {
                            _canjeables.push(`${r.name} <- ${campaignName} | url=${_url} | inst=${_inst.slice(0, 60)}`);
                        }
                        rewards.push({
                            name: r.name,
                            rewards: [r.name],
                            ...
                            //
                            // Y VUELVE A DECIRLO, desde el 2026-09-22. Entre el 09-11 y esa
                            // fecha `rewardGroups` no venia en la respuesta —no un array
                            // vacio: el campo no aparecia—, asi que `grupos` era siempre la
                            // caida al requisito de la campaña entera y esto valia 1
                            // SIEMPRE. Se anoto como «Twitch dejo de servirlo» y era falso:
                            // el campo seguia ahi y quien se habia quedado atras era NUESTRO
                            // hash de `ViewerDropsDashboard` (ver _gqlGetCampaigns).
                            //
                            // Mientras duro, el daño era este: una Great Ball se tachaba en
                            // cuanto su campaña concedia lo primero, aunque la pagina siguiera
                            // anunciando tres. Marcar de mas es esconder algo que aun se puede
                            // ganar, que es justo el error que este contador viene a evitar.
                            // Con el hash al dia vuelven los 1/3/3 medidos el 2026-09-08 y con
                            // ellos la cuenta buena, asi que una Great Ball con una sola
                            // concesion DEJA de salir tachada. Es un cambio visible y es el
                            // correcto: quedan dos cajas por abrir.
                ...
                // CUANTOS TRAMOS DE ESTA CAMPAÑA SE LLAMAN IGUAL, para el cruce por nombre de
                // _isDropClaimed: con tres «Great Ball» en la misma campaña, una concesion
                // llamada asi no dice cual de las tres fue.
                const _porNombre = {};
                for (const d of rewards) {
                    const k = _fold(String(d.name || '').toLowerCase());
                    _porNombre[k] = (_porNombre[k] || 0) + 1;
                }
                for (const d of rewards) d.homonimosEnCampaña = _porNombre[_fold(String(d.name || '').toLowerCase())] || 1;
                ...
                if (rewards.length > 0) {
                    ...
                    if (!_apiDropNames[key]) {
                        _apiDropNames[key] = { drops: [], startAt: rc.startsAt || '', endAt: rc.endsAt || '', displayTitle: key, imgSrc: _apiImage(rc), boxArt: _boxArtUrlOf(rc), searchText: textoEtiquetas, gameId: _gameIdOf(rc), brand, perCampaign: true, campaignId: rc.id || '' };
                    } else {
                        _mergeSearchText(_apiDropNames[key], textoEtiquetas);
                    }
            ...
            // Una campaña de recompensas puede tener un solo escalon de verdad, asi que
            // esto no afirma que falte nada: dice DONDE mirar si una tarjeta enseña menos
            // de lo que enseña la pagina. Si salen todas, el hash de `ViewerDropsDashboard`
            // es el primer sospechoso (ver _gqlGetCampaigns).
            if (_sinEscalones.length) {
                console.log('[Twitch Drops] campañas de recompensas sin rewardGroups (caen a su recompensa suelta):',
                    _sinEscalones.length, '->', _sinEscalones.join(', '));
            }
            ...
            for (const campaign of dropCampaigns) {
                ...
                try {
                    ...
                    for (const drop of timeBasedDrops) {
                        for (const be of (drop.benefitEdges || [])) {
                            const _t = be.benefit?.distributionType || '(sin campo)';
                            _censoTipos[_t] = (_censoTipos[_t] || 0) + 1;
                        }
                    ...
                    if (drops.length > 0) {
                        if (!_apiDropNames[apiKey]) {
                            ...
                            _apiDropNames[apiKey] = { drops: [], startAt: campaign.startAt || '', endAt: campaign.endAt || '', displayTitle, imgSrc: _apiImage(campaign), boxArt: _boxArtUrlOf(campaign), campaignId: campaign.id || '', searchText: textoEtiquetas, gameId: _gameIdOf(campaign), soloHermana: !casaSola };
                        } else {
                            _mergeSearchText(_apiDropNames[apiKey], textoEtiquetas);
            ...
            // Una linea corta y siempre: lo que interesa es ver aparecer una clave nueva.
            console.log('[Twitch Drops] tipos de recompensa:', JSON.stringify(_censoTipos));
            // Y esta solo cuando hay algo que decir, como la de los escalones que faltan.
            if (_canjeables.length) {
                console.log('[Twitch Drops] recompensas que se CANJEAN (candidatas a no borrar su aviso):',
                    _canjeables.length, _canjeables);
            }
        }
        ...
        // `compararMitades` apaga la ultima rama —la que cruza lo que hay antes del « - »
        // de los dos lados— para las entradas indexadas POR CAMPAÑA: ahi esa mitad es el
        // nombre de la campaña y no el del juego, asi que compararla con la del titulo es
        // comparar dos cosas distintas (ver _campaignKeyScore).
        function _titleScore(ct, k, compararMitades = true) {
            if (!k) return 0;
        ...
        // LAS ENTRADAS INDEXADAS POR CAMPAÑA SE PUNTUAN APARTE, y no con _titleScore. Su
        // clave es «<campaña> - <juego>», asi que la mitad de delante es el nombre de la
        // CAMPAÑA; la ultima rama de _titleScore compara las mitades de delante DANDO POR
        // HECHO que las dos son el juego, y ahi eso es falso.
        //
        // Lo que costaba: con «CONTROL Resonant launch - CONTROL Resonant» en el mapa, la
        // fila del JUEGO —«CONTROL Resonant - Twitch Gaming»— sacaba con ella la misma
        // puntuacion que con su propia entrada («control resonant», 16 letras), y al
        // empatar ganaba la que se hubiera guardado antes: las campañas de recompensas se
        // recorren primero, asi que la tarjeta del juego enseñaba los premios de la
        // campaña de recompensas.
        //
        // Lo unico que se quita es esa rama. Quitar ademas las subcadenas fue de mas y lo
        // canto test-panel-antes-de-las-filas: la campaña «Pokémon» —sin `game` y sin
        // `brand`, asi que su clave es el nombre pelado— la encuentra su fila «Pokémon -
        // Pokemon» justamente por ahi, y sin eso la fila se quedaba sin marca de coste.
        function _campaignKeyScore(ct, key) {
            const k = String(key || '').toLowerCase().trim();
            if (!k) return 0;
            // El nombre de la campaña ENTERO, que es lo que la pagina escribe en la
            // cabecera de esas filas (un solo <p>). Va por delante de todo lo demas
            // porque es la identidad exacta de la entrada, no un parecido.
            const campaña = k.split(' - ')[0].trim();
            if (campaña && _fold(ct) === _fold(campaña)) return 900;
            // Y el resto de la puntuacion de siempre menos la rama de las mitades: una
            // fila puede llevar pegado el estudio que imprime la pagina («Pokémon -
            // Pokemon» contra la campaña «Pokémon») y tiene que seguir encontrandose.
            return _titleScore(ct, k, false);
        }
        ...
        function _findEntryForTitle(cardTitle) {
            ...
            for (const [key, entry] of Object.entries(_apiDropNames)) {
                ...
                const score = entry.perCampaign
                    ? Math.max(_campaignKeyScore(ct, key), _brandScore(ct, entry.brand))
                    : Math.max(
        ...
        function _appendDropNamesTo(card, drops) {
            ...
            for (const d of drops) {
                ...
                for (const r of (_earnedRewardsByCampaign[d.campaignId] || [])) {
                    ...
                    // Si es la recompensa de un tramo y ese tramo ya sale tachado, ya esta
                    // dicho: pintarlo aqui otra vez era el «✓ Sierra Suit» duplicado de
                    // CONTROL. Si el tramo NO se pudo tachar (homonimos), se deja: es lo
                    // unico que dice que ya te llevaste una.
                    const rn = _fold(r.name.toLowerCase());
                    if (drops.some(x => x && x.rewardCampaign && x.campaignId === d.campaignId &&
                        _fold(String(x.name || '').toLowerCase()) === rn && _isDropClaimed(x) === true)) continue;
        ...
        function showInfoModal() {
            // En este script `t` es i18n[lang] || i18n.en, sin merge, asi que una clave
            // que le falte a un idioma sale como undefined en pantalla en vez de caer al
            // ingles. Todas las de este modal estan traducidas en los 16, pero se
            // resuelven por aqui para que anadir una nueva no pueda romper la vista.
            const tx = (key) => t[key] || i18n.en[key];
            const { overlay, box } = createModalContainer();
            // Este modal es el unico con contenido de largo imprevisible (la descripcion
            // pasa de 2700 caracteres), asi que en vez de dejar que scrollee la caja
            ...
            titleEl.textContent = tx('scriptInfoTitle');
            ...
            [
                { label: tx('scriptInfoName'), value: "Twitch Drops Highlighter + Keywords (Full + i18n)" },
                { label: tx('scriptInfoVersion'), value: SCRIPT_VERSION },
                { label: tx('scriptInfoAuthor'), value: "g31w0fw0rld" },
                { label: tx('scriptInfoGitHub'), value: "github.com/g31w0fw0rld/twitch-drops-highlighter", isLink: true },
            ...
            [
                { title: tx('scriptInfoDescription'), text: tx('scriptInfoDescriptionText') },
                { title: tx('scriptInfoPrivacy'), text: tx('scriptInfoPrivacyText') }
            ].forEach((s, i) => {
                ...
                Object.assign(h.style, {
                    fontWeight: 'bold', color: colors.purpleLight,
            ...
            const detach = attachDismissHandlers(overlay, () => { closeOverlayAnimated(overlay); });
            const closeBtn = createButton(tx('accept'), colors.purple, () => {
                detach();
        ...
        async function highlightAndLinkDrops() {
            ...
            allNodes.forEach((node, index) => {
                ...
                // _domTitleByGameId). Se apunta de todas las filas que casen o no: antes
                // solo se aprendia de las que ya casaban, y el alias sirve justamente
                // para reconocer las que su propio texto no delata.
                //
                // PERO SOLO DE LAS FILAS QUE DICEN DE QUE JUEGO SON, que son las que
                // traen DOS <p>: el juego y el estudio. La cabecera de un solo <p> se
                // titula con el nombre de la CAMPAÑA, y apuntar eso como apodo del juego
                // es meter una mentira en el mapa que decide que entrada de la API le
                // toca a cada fila.
                //
                // Reportado el 2026-09-21 con CONTROL Resonant, que tiene dos campañas
                // abiertas —una de drops y una de recompensas— y las dos se llaman
                // «CONTROL Resonant launch». La fila de recompensas trae un solo <p> con
                // ese nombre, asi que el juego 1338428218 quedaba apodado «control
                // resonant launch»; con ese apodo, _findEntryForTitle le daba a la fila
                // la entrada de la campaña de DROPS con puntuacion de coincidencia
                // exacta, y la fila enseñaba los premios de la otra campaña (y su coste:
                // «⏱ 1h» donde lo suyo son 4 h). De paso, la entrada de la API se
                // consideraba ya vista por ese mismo apodo y la tarjeta del juego
                // desaparecia del panel.
                //
                // Las tres filas reales verificadas dicen lo mismo:
                //   · «Pokémon» + «Pokemon»            (2 <p>) -> el primero ES el juego
                //   · «CONTROL Resonant» + «Twitch Gaming» (2) -> el primero ES el juego
                //   · «Treasure Hunt Drop»             (1 <p>) -> es el nombre de la campaña
                // Y el caso que justifica el mapa entero —«Eventos especiales - Twitch
                // Gaming», el juego que Twitch traduce— lleva sus dos <p>, asi que sigue
                // aprendiendose. No se usa el `alt` de la caratula para decidirlo: ahi
                // pone el nombre del juego, pero no esta comprobado que la pagina lo
                // traduzca igual que el titulo, y si no lo tradujera esta comparacion
                // dejaria fuera justamente el caso para el que existe el alias.
                const domGameId = _gameIdFromBoxArt(imgSrc);
                if (domGameId && studioText) _domTitleByGameId[domGameId] = displayTitle.toLowerCase();
        ...
        // EL ENLACE DE CATEGORIA DE UNA CAMPAÑA DE RECOMPENSAS, que Twitch escribe a secas.
        //
        // Las campañas de drops enlazan `/directory/category/<slug>?filter=drops` y de eso
        // se ocupa lo de arriba. Las de recompensas casi nunca enlazan nada —Minecraft y
        // Pokemon no nombran categoria—, pero la de CONTROL Resonant (2026-09-23) dice
        // «Mira 60 minutos de un canal participante [...] en la categoria CONTROL
        // Resonant» y enlaza `/directory/category/control-2` SIN filtro: la categoria
        // entera, participe cada canal o no. Es la misma queja que origino el filtrado de
        // los de drops, asi que recibe el mismo trato: `filter=drops&dropID=<su id>`, y la
        // negrita de siempre.
        //
        // De que campaña es el enlace no lo dice el bloque: sus `<strong>` son los minutos
        // («60 minutos»), no un nombre, asi que `_nombreDeSuCampaña` devolveria eso. Lo
        // dice la CABECERA del acordeon —la fila de recompensas se titula con el nombre de
        // la campaña, un solo <p>, ver el escaneo— y la caratula, que da el juego.
        //
        // TRES GUARDAS, por lo mismo que las de arriba: mandar a la campaña equivocada es
        // peor que no tocar nada.
        //   · el acordeon no puede traer ningun enlace con `filter=drops`: eso es un
        //     acordeon de DROPS y lo que hubiera ahi a secas no es de esta regla.
        //   · la cabecera tiene que nombrar una campaña de recompensas que la API conozca,
        //     y de ESE juego si la caratula lo dice.
        //   · y si no se encuentra, el enlace se queda como lo escribio Twitch.
        //
        // VERIFICADO EN EL NAVEGADOR el 2026-09-23 con CONTROL Resonant: el directorio
        // acepta el id de una campaña de RECOMPENSAS en `dropID`, no falla ni sale vacio.
        // Ese dia el listado era EL MISMO que sin el filtro, asi que no se puede decir que
        // lo acote —el README no lo promete—; se deja porque apunta a lo que dice la frase
        // de la pagina, y el dia que la campaña no abarque a toda la categoria sera la
        // diferencia.
        const DIR_LINK_REWARD_ATTR = 'data-drop-dir-reward';
        function _rewardIdDeSuAcordeon(a) {
            let n = a.parentElement;
            for (let i = 0; i < 14 && n; i++, n = n.parentElement) {
                const cab = n.querySelector('div.accordion-header');
                if (!cab) continue;
                // Sin contar los que ya reescribio esta misma regla: tras el primero de
                // los tres enlaces de CONTROL, los otros dos veian un `filter=drops` en su
                // acordeon —el nuestro— y se quedaban sin tocar.
                if (n.querySelector('a[href*="/directory/category/"][href*="filter=drops"]:not([' + DIR_LINK_REWARD_ATTR + '])')) return '';
                const p = cab.querySelector('p');
                const clave = _claveDeCampaña(p ? p.textContent : '');
                if (!clave) return '';
                const img = cab.querySelector('img[src*="ttv-boxart"]');
                const gid = img ? _gameIdFromBoxArt(img.getAttribute('src')) : '';
                return (gid ? _apiRewardCampaignIdsPorJuego[_claveDeCampañaEnJuego(gid, clave)]
                            : _apiRewardCampaignIdsSinJuego[clave]) || '';
            }
            return '';
        }
        ...
        function _filtrarEnlacesDeCanales(root) {
            ...
            (root || document).querySelectorAll(DIR_LINK_SEL).forEach((a) => {
                ...
                if (!/[?&]filter=drops\b/.test(href)) {
                    // Un `dropID` que no venga con `filter=drops` no lo pusimos nosotros.
                    if (/[?&]dropID=/.test(href)) return;
                    const idRc = _rewardIdDeSuAcordeon(a);
                    if (!idRc) return;
                    // Sin modal, a proposito: la lista de canales sale de
                    // `DropCampaignDetails`, que es de campañas de drops, y preguntarle por
                    // una de recompensas es una consulta que no puede contestar.
                    a.setAttribute('href', href + (href.indexOf('?') === -1 ? '?' : '&') +
                        'filter=drops&dropID=' + encodeURIComponent(idRc));
                    _ensureDirLinkStyle();
                    a.setAttribute(DIR_LINK_ATTR, '1');
                    a.setAttribute(DIR_LINK_REWARD_ATTR, '1');
                    hechos++;
                    return;
                }
        ...
        // LA CAMPAÑA QUE EL PANEL YA NO LISTA, PERO SIGUE EN TU INVENTARIO.
        //
        // `_apiCampaignStatusById` se llena con lo que devuelve `ViewerDropsDashboard`, y
        // esa consulta deja de traer una campaña poco despues de que cierre. El inventario
        // no: lo que ganaste ahi se queda. Asi que hay una franja —los dias siguientes al
        // cierre— en la que el bloque esta delante y el indice no sabe nada de el, y
        // `_campañaCaducada` contesta que no, que es la direccion segura pero deja la
        // campaña a la vista con la casilla puesta.
        //
        // Medido el 2026-09-22 con «PEC: Fall Finals 1_DAY3» (afe7cb0a-…) y «Jynxzi
        // Invitational» (703bea65-…), las dos terminadas el dia 20: de las tres campañas
        // que el barrido dejaba a la vista, esas dos NO estaban entre los 122 ids del
        // indice, y la tercera —«OWCS Stage 3 Asia Kickoff»— si estaba y daba `active`,
        // que es correcto y por eso se queda.
        //
        // Se PREGUNTA por ese id en vez de leer la pagina. El bloque dice que cerro de dos
        // formas y las dos estan traducidas a 28 locales —la fecha («Fecha de termino: dom
        // 20 de sep») y la frase («Esta recompensa ya no esta disponible»)—, asi que
        // acertarlas seria adivinar en 26 idiomas que no se pueden comprobar desde aqui. El
        // id, en cambio, esta escrito en el enlace del propio bloque y no se traduce.
        //
        // Una consulta por campaña desconocida, no por campaña: lo normal es cero —el
        // indice trae 122— y aqui fueron dos. Y solo con la casilla puesta y la API ya
        // contestada, para no preguntar por algo que el dashboard todavia podia responder.
        //
        // Si la respuesta no trae ni `status` ni `endAt`, no se escribe nada y se dice en
        // consola: es una persisted query y su seleccion de campos la fija Twitch, asi que
        // el dia que deje de traerlos hay que verlo, no deducirlo de que ya nada se esconde.
        const _estadoPreguntado = new Set();

        async function _preguntarEstadoDeCampaña(id) {
            if (!id || _estadoPreguntado.has(id)) return;
            _estadoPreguntado.add(id);
            let c = null;
            try {
                // `channelLogin` va al respaldo de siempre: la campaña se busca por su
                // dropID y ese parametro solo da contexto (ver _gqlGetCampaigns).
                const d = await _gqlGetCampaignDetails(id, 'twitch');
                c = d?.user?.dropCampaign || null;
            } catch (e) { /* sin respuesta no se opina */ }
            if (!c) {
                console.warn('[Twitch Drops] la campaña', id, 'no esta en el panel y su detalle'
                    + ' no respondio: se queda a la vista.');
                return;
            }
            if (!c.status && !c.endAt) {
                console.warn('[Twitch Drops] el detalle de', c.name || id, 'llega sin status ni'
                    + ' endAt: no se puede saber si cerro, se queda a la vista.');
                return;
            }
            const estado = _campaignStatus(c);
            console.log('[Twitch Drops] campaña fuera del panel:', c.name || id, '->', estado,
                '| status:', c.status || '(no viene)', '| endAt:', c.endAt || '(no viene)');
            _apiCampaignStatusById[id] = estado;
            // El barrido ya paso: se repite para que la respuesta surta efecto ahora y no
            // en la siguiente vuelta, que puede no haberla. El guardia de arriba impide
            // que esto se llame a si mismo en bucle.
            _esconderCampañasCaducadas();
        }
        ...
        function _esconderCampañasCaducadas() {
            ...
            for (const img of _inventoryImages()) {
                ...
                if (!_campañaCaducada(container)) {
                    // Que no conste cerrada tiene DOS motivos que no son lo mismo: que
                    // este abierta —y entonces no hay nada que hacer— o que el indice no
                    // sepa nada de ella. Solo el segundo se pregunta.
                    const id = _idDeCampañaDelBloque(container);
                    if (_apiDataReady && id && !(id in _apiCampaignStatusById)) {
                        _preguntarEstadoDeCampaña(id);
                    }
                    continue;
                }
        ...
        function cleanInventory(type = "expired") {
            ...
            const checkNotifications = function (dropTextArrayVar) {
                ...
                setTimeout(() => {
                    ...
                    //
                    // Los demas avisos de drop se barren porque son redundantes: lo que
                    // anuncian ya esta en tu inventario y ahi se queda. El de un codigo no
                    // lo es —lo que trae es el canje— asi que borrarlo es la unica forma
                    // que tiene este script de hacerte perder algo.
                    //
                    // Va ANTES que todo lo demas, incluido el marcador de emote/emblema:
                    // este es un veto, no un criterio mas que pesar.
                    const esDeCodigo = (n) => {
                        if (!_codeRewards.length) return false;
                        const texto = _fold((n.innerText || n.textContent || '').toLowerCase());
                        for (const c of _codeRewards) {
                            if (c.nombre && texto.includes(c.nombre)) return true;
                            if (c.asset && n.querySelector(`img[src*="${c.asset}"]`)) return true;
                        }
                        return false;
                    };
                    const toDelete = Array.from(document.querySelectorAll('.persistent-notification')).filter((n) => {
                        ...
                        // textContent de respaldo: `innerText` es propiedad del renderizado
                        // y no existe fuera de un navegador de verdad. Sin esto, este
                        // filtro no se puede ejercitar en ninguna prueba.
                        const notifText = (body.innerText || body.textContent || '').toLowerCase();
            ...
            // SIN EL HISTORIAL NO SE BARRE NADA, y esta guarda es la mitad del arreglo de
            // arriba: `_codeRewards` se llena con la respuesta de `Inventory`, que es
            // asincrona, asi que un barrido lanzado a los 2 s podia encontrarse la lista
            // vacia y borrar el aviso del codigo por pura carrera. Borrar es lo unico
            // irreversible que hace este script, asi que sin datos no lo hace: se espera,
            // y si el inventario no llega, no se barre. Quedarse con avisos de mas es un
            // estorbo; quedarse sin uno es perder un codigo.
            const _conInventarioListo = (fn, intento = 0) => {
                if (_claimedIndexReady) { fn(); return; }
                if (intento >= 12) {
                    console.warn('[Twitch Drops] sin inventario: no se barren las notificaciones'
                        + ' (no se puede saber cual lleva un codigo).');
                    return;
                }
                setTimeout(() => _conInventarioListo(fn, intento + 1), 1000);
            };
